						...
						<div class="form-group table-responsive">
							...
							<table class="table table-bordered" id="item_table">
								...
								<tr>
									...
									<td><select class="existingColor" style="width: 100%;padding:3%" name="product_color[<?php echo $i ?>][]" multiple
											required>
											<?php echo updateSelectedColor($colorArray, $color);?>
										</select>
									</td>
							...
							<script>
								$(document).ready(function () {
									...
									var i = document.getElementsByClassName("existingColor").length;
...
function updateSelectedOptions($sizeArray, $selectedSize){
  ...
  function updateSelectedColor($colorArray, $selectedColor){
	$options='';
	foreach($colorArray as $itemColor){
		$options.='<option style="padding-left: 10%;padding-top:5%;" value="'.$itemColor.'" ';
		if(in_array($itemColor,$selectedColor)) 
			$options.= " selected";
		$options.= '>'.$itemColor.'</option>';
	}
	return $options;
  }
...
if (isset($_POST['submit'])) {
	...
	// }
	$fristColor = implode(",",$product_color[0]);
    $sql = "UPDATE product 
    ...
    if (! $query) {
		...
		foreach ($product_color as $Color){
			$selectedColor = implode(',',$Color);
			$sql = "INSERT into product_size (id, product_id, size, color, price_per_item, items_per_set)
			values(null, '$id','$productSize[$i]','$selectedColor', '$PricePerItem[$i]', '$ItemPerSet[$i]' )";
			$query = mysqli_query($con,$sql);
			$i=$i+1;
		}
